...
class FrameExtractorPrivate;
class Q_AV_EXPORT FrameExtractor : public QObject {
	Q_OBJECT
	DPTR_DECLARE_PRIVATE(FrameExtractor)
 ...
 public:
	explicit FrameExtractor(QObject *parent = 0);
	/*!
	 * \brief setSource
 ...
 Q_SIGNALS:
	...
	void asyncChanged();
	void
	error(const QString &errorMessage); ///< emitted with a helpful error message -- connect to this to show empty image in preview widget
	void aborted(const QString &abortMessage); ///< emitted when aborting the current preview -- if user requested a new preview this usually
	                                           ///< gets emitted.  connect to this to show empty preview
