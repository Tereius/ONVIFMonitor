 ...
 public:
	...
	void addTask(QRunnable *t) {
		...
		while(tasks.size() >= tasks.capacity() && tasks.capacity() > 0) {
			...
			if(task && task->autoDelete()) delete task;
		}
 ...
 protected:
	virtual void run() {
		while(!stop) {
			...
			if(task) {
				task->run();
				if(task->autoDelete()) delete task;
			}
		}
...
class FrameExtractorPrivate : public DPtrPrivate<FrameExtractor> {
 public:
	FrameExtractorPrivate() :
	 abort_seek(false),
	 async(true),
	 has_video(true),
	 auto_extract(true),
	...
	bool checkAndOpen() {
		...
		demuxer.setStreamIndex(QtAV::AVDemuxer::VideoStream, 0);
		foreach(const QString &c, codecs) {
			QtAV::VideoDecoder *vd = QtAV::VideoDecoder::create(c.toUtf8().constData());
			if(!vd) continue;
			decoder.reset(vd);
			...
			QVariantHash opt, va;
			// FIXME: why QStringLiteral can't be used as key for vs<2015 but somewhere else it can?  error C2958: the left bracket '[' found at
			// qstringliteral
	...
	bool extractInPrecision(qint64 value, int range, QString &err, bool &aborted) {
		...
		err = "";
		aborted = false;
		frame = QtAV::VideoFrame();
		if(value < demuxer.startTime()) value += demuxer.startTime();
		// demuxer.seek(value);
		...
		while(!demuxer.atEnd()) {
			...
			if(!demuxer.readFrame()) continue;
			if(demuxer.stream() != vstream) continue;
			pkt = demuxer.packet();
			if(pts0 < 0LL) pts0 = (qint64)(pkt.pts * 1000.0);
			if((qint64)(pkt.pts * 1000.0) - value > (qint64)range) {
				if(warn_out_of_range) qDebug("read packet out of range");
			...
			if(pkt.hasKeyFrame) break;
			if(warn_bad_seek) qWarning("Not seek to key frame!!!");
			warn_bad_seek = false;
		}
		...
		while(!demuxer.atEnd()) {
			...
			if(dec_opt != dec_opt_old) decoder->setOptions(*dec_opt);
			// invalid packet?
			if(!decoder->decode(pkt)) {
			...
			const QtAV::VideoFrame f = decoder->frame();
			if(!f.isValid()) {
		...
		if(demuxer.atEnd()) releaseResourceInternal(false);
		return true;
	}
	void releaseResourceInternal(bool releaseFrame = true) {
		if(releaseFrame) frame = QtAV::VideoFrame();
		seek_count = 0;
	...
	void safeReleaseResource() {
		class Cleaner : public QRunnable {
			FrameExtractorPrivate *p;

		 public:
			Cleaner(FrameExtractorPrivate *pri) : p(pri) {}
			void run() { p->releaseResourceInternal(); }
		};
	...
	volatile int precision; ///< is volatile because may be written by this->thread() and read by extractor thread but typical use is to not
	                        ///< modify it while extract is running
	QString source; ///< is written-to by this->thread() but may be read by extractor thread; important: apparently two threads accessing
	                ///< QString is supported by Qt according to wang-bin, so we aren't guarding this with a lock
	QtAV::AVDemuxer demuxer;
	QScopedPointer<QtAV::VideoDecoder> decoder;
	QtAV::VideoFrame frame; ///< important: we only allow the extract thread to modify this value
	QStringList codecs;
	ExtractThread thread;
	static QVariantHash dec_opt_framedrop, dec_opt_normal;
};

QVariantHash FrameExtractorPrivate::dec_opt_framedrop;
QVariantHash FrameExtractorPrivate::dec_opt_normal;

FrameExtractor::FrameExtractor(QObject *parent) : QObject(parent) {
	DPTR_D(FrameExtractor);
	d.thread.start();
}

void FrameExtractor::setSource(const QString url) {
	DPTR_D(FrameExtractor);
	if(url == d.source) return;
	d.source = url;
	d.has_video = true;
...
QString FrameExtractor::source() const {
	return d_func().source;
}

void FrameExtractor::setAsync(bool value) {
	DPTR_D(FrameExtractor);
	if(d.async == value) return;
	d.async = value;
	Q_EMIT asyncChanged();
}

bool FrameExtractor::async() const {
	return d_func().async;
}

void FrameExtractor::setAutoExtract(bool value) {
	DPTR_D(FrameExtractor);
	if(d.auto_extract == value) return;
	d.auto_extract = value;
	Q_EMIT autoExtractChanged();
}

bool FrameExtractor::autoExtract() const {
	return d_func().auto_extract;
}

void FrameExtractor::setPosition(qint64 value) {
	DPTR_D(FrameExtractor);
...
qint64 FrameExtractor::position() const {
	return d_func().position;
}

void FrameExtractor::setPrecision(int value) {
	DPTR_D(FrameExtractor);
	if(d.precision == value) return;
...
int FrameExtractor::precision() const {
...
void FrameExtractor::extract() {
	...
	class ExtractTask : public QRunnable {
	 public:
		ExtractTask(FrameExtractor *e, qint64 t) : extractor(e), position(t) {}
		void run() { extractor->extractInternal(position); }
...
void FrameExtractor::extractInternal(qint64 pos) {
	DPTR_D(FrameExtractor);
