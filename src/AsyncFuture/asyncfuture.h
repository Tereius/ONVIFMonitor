...
#define ASYNC_FUTURE_CALLBACK_STATIC_ASSERT(Tag, Completed)                                                                    \
	...
	static_assert(Private::decay_is_same<T, Private::Arg0Type<Completed>>::value || Private::arg0_is_future<Completed>::value || \
...
namespace Private {
	...
	void watch(QFuture<T> future, const QObject *owner, const QObject *contextObject, Finished finished, Canceled canceled, Progress progress,
		...
		if(contextObject) {
			...
			QObject::connect(watcher, &QFutureWatcher<T>::progressValueChanged, contextObject, [=](int value) { progress(value); });
			...
			QObject::connect(watcher, &QFutureWatcher<T>::progressValueChanged, [=](int value) { progress(value); });

			QObject::connect(watcher, &QFutureWatcher<T>::progressRangeChanged, [=](int min, int max) { progressRange(min, max); });
		}
	 ...
	 *
	 * 2) Its member function do not use <T> to avoid to use template specialization to handle <void>. Type checking should be done by user
	 * classes (e.g Deferred)
	 *
	 */
	...
	class DeferredFuture : public QObject, public QFutureInterface<T> {
	 public:
		...
		void track(QFuture<ANY> future) {
			...
			QObject::connect(watcher, &QFutureWatcher<ANY>::suspending, this, [=]() { thiz->future().toggleSuspended(); });
#elif QT_VERSION >= 0x050000
			QObject::connect(watcher, &QFutureWatcher<ANY>::paused, this, [=]() { thiz->future().togglePaused(); });
#endif

			QObject::connect(watcher, &QFutureWatcher<ANY>::resumed, this, [=]() { thiz->future().resume(); });
		...
		void complete(QFuture<T> future) {
			...
			auto onFinished = [strongRef, future]() { strongRef->template completeByFinishedFuture<T>(future); };

			auto onCanceled = [strongRef]() { strongRef->cancel(); };

			watch(future, this, nullptr, onFinished, onCanceled, [](int) {}, [](int, int) {});
			...
			watch(this->future(), this, nullptr, []() {}, pushCancel, [](int) {}, [](int, int) {});
		...
		void complete(QFuture<QFuture<ANY>> future) {
			auto strongRef = this->weakRef.toStrongRef();
			auto onFinished = [strongRef, future]() { strongRef->complete(future.result()); };
		...
		void cancel(const QObject *sender, Member member) {
			...
			QObject::connect(sender, member, this, [weakRef_]() {
		...
		void cancel(QFuture<ANY> future) {
			auto strongRef = this->weakRef.toStrongRef();
			auto onFinished = [strongRef]() { strongRef->cancel(); };

			watch(future, this, nullptr, onFinished, []() {}, [](int) {}, [](int, int) {});
		}
	 ...
	 protected:
		DeferredFuture(QObject *parent = nullptr) : QObject(parent), QFutureInterface<T>(QFutureInterface<T>::Running) {
	...
	class CombinedFuture : public DeferredFuture<void> {
	 ...
	 public:
		CombinedFuture(bool settleAllModeArg = false) :
		 DeferredFuture<void>(), settledCount(0), count(0), anyCanceled(false), settleAllMode(settleAllModeArg) {
		...
		void addFuture(const QFuture<T> future) {
			...
			auto strongRef = this->weakRef.toStrongRef();
			Private::watch(
			 future, this, 0, [strongRef, index]() { strongRef->completeFutureAt(index); },
			 [strongRef, index]() { strongRef->cancelFutureAt(index); }, progressFunc, progressRangeFunc);
		}
	 ...
	 private:
		class FutureInfo {
		 public:
			FutureInfo() = default;
			FutureInfo(QFuture<void> childFuture) : childFuture(childFuture) {}
		...
		void updateProgressRange() {
			int max = std::accumulate(futures.begin(), futures.end(), 0, [](int current, const FutureInfo *info) { return info->max + current; });
		...
		void updateProgress() {
			int value =
			 std::accumulate(futures.begin(), futures.end(), 0, [](int current, const FutureInfo *info) { return info->value + current; });
	...
	struct is_qfuture : std::false_type {};
	template<typename T>
	struct is_qfuture<QFuture<T>> : std::true_type {};
...
// Case 2: Functor takes T directly
	...
	template<typename Functor, typename T>
	auto callIgnoreReturn(Functor &functor, QFuture<T> value)
	 -> std::enable_if_t<!std::is_invocable_v<Functor, QFuture<T>> && !std::is_invocable_v<Functor, T>, void> {
...
// Case 2: Functor takes T directly
#if QT_VERSION >= 0x060000
	template<typename Functor, typename T>
	auto call(Functor &functor, QFuture<T> value) -> std::enable_if_t<std::is_invocable_v<Functor, T>, CallerRetType<Functor, T>> {
	...
	template<typename Functor, typename T>
	typename std::enable_if<ret_type_is_void<Functor>::value && arg_count_is_zero<Functor>::value, Value<RetType<Functor>>>::type
	...
	class CancelOnce {
	 public:
		CancelOnce(Canceled onCanceled) : onCanceled(onCanceled) {}
...
class Observable {
 ...
 public:
	...
	typename std::enable_if<
	 ...
	 Observable<typename Private::future_traits<typename Private::function_traits<Completed>::result_type>::arg_type>>::type
	context(const QObject *contextObject, Completed functor) {
	...
	context(const QObject *contextObject, Completed onCompleted, Canceled onCanceled) {
		...
		return _context<typename Private::future_traits<typename Private::function_traits<Completed>::result_type>::arg_type,
		                typename Private::function_traits<Completed>::result_type>(contextObject, onCompleted, onCanceled);
	}

	/* subscribe function */

	template<typename Completed, typename Canceled>
	typename std::enable_if<!Private::ret_type_is_future<Completed>::value, Observable<typename Private::RetType<Completed>>>::type
	...
	template<typename Functor>
	typename std::enable_if<std::is_same<typename Private::RetType<Functor>, void>::value, void>::type onProgress(Functor functor) {
		onProgress([=]() mutable {
			functor();
			return true;
		});
	...
	template<typename Functor>
	typename std::enable_if<std::is_same<typename Private::RetType<Functor>, bool>::value, void>::type onProgress(Functor onProgressArg) {
 ...
 private:
	...
	Observable<ObservableType> _context(const QObject *contextObject, Completed onCompleted, Canceled onCanceled) {
		...
		auto future = Private::execute<ObservableType, RetType>(m_future, contextObject, onCompleted, onCanceled);
	...
	Observable<ObservableType> _subscribe(Completed onCompleted, Canceled onCanceled) {
		...
		return _context<ObservableType, RetType, Completed, Canceled>(QCoreApplication::instance(), onCompleted, onCanceled);
	}
};
...
class Deferred : public Observable<T> {
 ...
 public:
	Deferred() : Observable<T>(), deferredFuture(Private::DeferredFuture<T>::create()) { this->m_future = deferredFuture->future(); }
	...
	void setProgressRange(int minimum, int maximum) { deferredFuture->setProgressRange(minimum, maximum); }
...
class Deferred<void> : public Observable<void> {
 ...
 public:
	Deferred() : Observable<void>(), deferredFuture(Private::DeferredFuture<void>::create()) { this->m_future = deferredFuture->future(); }
...
template<typename Member>
auto observe(QObject *object, Member pointToMemberFunction) -> Observable<typename Private::signal_traits<Member>::result_type> {
