...
Uuid DeviceManager::addDevice(const QUrl &rEndpoint, const QString &rDeviceName /*= QString()*/) {
	...
	return addDevice(rEndpoint, QString(), QString(), false, rDeviceName);
}

Uuid DeviceManager::addDevice(const QUrl &rEndpoint, const QString &rUsername, const QString &rPassword, bool save /*= false*/, const QString &rDeviceName /*= QString()*/) {

	auto deviceId = Uuid::createUuid();
	if(rEndpoint.isValid()) {
		...
		QtConcurrent::run([=]() {
			...
			settings.setValue("id", deviceId.toQuuid());
			...
			else if(result.getErrorCode() == RESULT_UNAUTHORIZED) {
				emit unauthorized(deviceId);
			}
		});
	}
	return deviceId;
