...
class BlockingQueue {
 public:
	 ...
	 * \param isValid a pointer to a bool (optional).  If isValid is set to true after a call, the returned item is valid. False means the
	 * queue was empty or the timeout expired. \return the item taken.  It may not be valid if the queue was empty and timeout expired. Check
	 * optional isValid flag to determine if that is the case.
	 */
...
BlockingQueue<T, Container>::BlockingQueue() :
 block_empty(true), block_full(true), cap(48), thres(32), empty_callback(0), threshold_callback(0), full_callback(0) {}
...
bool BlockingQueue<T, Container>::put(const T &t, unsigned long timeout_ms) {
	...
	if(checkFull()) {
		...
		if(block_full) ret = cond_full.wait(&lock, timeout_ms);
...
T BlockingQueue<T, Container>::take(unsigned long timeout_ms, bool *isValid) {
	...
	if(checkEmpty()) { // TODO:always block?
		...
		if(block_empty) cond_empty.wait(&lock, timeout_ms); // block when empty only
	}
	if(checkEmpty()) {
