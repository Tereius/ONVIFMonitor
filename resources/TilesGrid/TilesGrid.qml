            ...
            property TilesGridLogic model: TilesGridLogic {
                ...
                onRowAdded: row => {
                                ...
                                for (var i = 0; i < grid.columns; i++) {
                                    let indexToAdd = row * grid.columns + i
                                    tileModel.insert(indexToAdd, {})

                                    // Restore the visual tile (if there was one before)
                                    let tile = d.model.tileAt(indexToAdd)
                                    if (tile && tile.row === row
                                        && tile.column === i) {
                                        let visualTile = d.visualTiles[tile.id]
                                        let tileHolderTarget = grid.tileHolderAt(
                                            indexToAdd)
                                        if (visualTile && tileHolderTarget) {
                                            visualTile.x = 0
                                            visualTile.y = 0
                                            visualTile.parent = tileHolderTarget
                                        }
                                    }
                                }
                            }
                ...
                onRowRemoved: row => {
                                  ...
                                  for (var i = grid.columns - 1; i >= 0; i--) {
                                      let indexToRemove = row * grid.columns + i
                ...
                onColumnAdded: column => {
                                   ...
                                   for (var i = 0; i < grid.rows; i++) {
                                       let indexToAdd = i * grid.columns + column
                                       tileModel.insert(indexToAdd, {})
                ...
                onColumnRemoved: column => {
                                     ...
                                     for (var i = grid.rows - 1; i >= 0; i--) {
                                         let indexToRemove = i * grid.columns + column
                                         tileModel.remove(indexToRemove)
                                     }
    ...
    DropArea {
        ...
        function handleDrag(drag) {
            ...
            if (drag.source && drag.source instanceof Tile) {
                ...
                tileHolder = grid.tileHolderAtPosition(mappedPoint.x,
                                                       mappedPoint.y)
